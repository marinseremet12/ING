...
class HpEasyAdmin extends React.Component {
    ...
    async handleTransferAddressChange(event) {
        ...
        if (await Utils.isValidAddress(transferAddress)) {
            ...
            Swal.fire({
                ...
                html: 'Sorry, but provided input data <b>' + transferAddress + "</b> is not a valid BSC address"
            });
        }
    }
    ...
    async handleTransfer() {
        ...
        if (!this.state.transferAddress) {
            Swal.fire({
                ...
                text: 'Not valid recipient BSC address'
            })
            return;
        }
    ...
    async handleChangeOwner() {
        ...
        Swal.fire({
            ...
            showCancelButton: true,
            html: 'Are you sure you want to change <b>INFINITY GLOBAL APP contract</b> owner to <b>' + this.state.newOwnerCandidate + "</b> <br>(current contract owner is <i>" + this.props.owner + "</i>)"
        }).then((result) => {
            if (result.isConfirmed) {
                this.chageOwner();
            } else {
            ...
            .then(
                result => {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        html: 'Nice! INFINITY BNB contract ownership has been transfered to <b>' + this.state.newOwnerCandidate + '</b>. <br>New contract owner must login to this page and accept ownership, meanwhile <b>' + this.props.owner + '</b> will stay the contract owner<br>Current page will be reloaded'
                    }).then(result => {
                        this.changeLoader(false);
                        window.location.reload();
                    });
    ...
    render() {
        return (
            <div className="admin-sub-block">
                ...
                <h3>INFINITY BNB contract funds</h3>
                <div className="data-change-container">
                    <div className="subDiv">
                        <label>
                ...
                <h3>INFINITY BNB smartcontract owner</h3>
                <div className="data-change-container">
                    <div className="subDiv">
                        <label className="short">
                            CURRENT OWNER:
