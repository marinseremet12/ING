...
class SecondRegistrationStep extends React.Component {
    ...
    async handleSuccessfulSubmit(result) {
        Swal.fire({
            title: 'Success',
            icon: 'success',
            html: 'Welcome to the INFINITY BNB family! <br>You will be redirected to your dashboard'
        }).then((result) => {
            this.setState({ loader: false });
            this.props.history.push('/dashboard');
        });
    ...
    render() {
        let text = 'To complete your registration you need to ' + (this.state.isExtraTokenNeeded ? 'purchase ' + this.state.REGISTRATION_COST_ETH + 'IGL (cost ' + this.state.REGISTRATION_COST_EASY_ETH_EQUIVALENT / 1000000000000000000 + ' ETH) and send the to the INFINITY IGL smartcontract' : 'send ' + this.state.REGISTRATION_COST_ETH + ' IGL COIN to the INFINITY BNB smartcontract')
        if (!this.state.isEthPaid && this.state.isEasyPaid) {
            text = this.state.REGISTRATION_COST_ETH + ' IGL COIN is paid. Nice! To complete your registration you need to go to registration page and trasfer ' + this.state.REGISTRATION_COST_ETH + ' BNB to the INFINITY BNB smartcontract'
        }
