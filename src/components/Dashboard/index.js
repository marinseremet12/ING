...
class Dashboard extends React.Component {
    ...
    processSkippedMatrixResult() {
        ...
        if (this.state.skippedMatrices.length === this.state.userMatricesIds.length) {
            ...
            html += (isOneMatrix ? ' was' : ' were') + ' skipped. <p>To be able cycle again please purchase new INFINITY GLOBAL APP<br>';
            Swal.fire({
                icon: 'info',
    ...
    async checkJackpotPaymentStatus() {
        ...
        if (jackpotQueue.isEthPaid) {
            Swal.fire({
                ...
                html: 'We have noticed that you have already paid <b>' + jackpotQueue.betSize + "&nbsp;ETH</b> to join Jackpot <b>line&nbsp;" + jackpotQueue.line + "</b>.</p> <p>Only one step's left to join this jackpot line: <br><b>" + jackpotQueue.betSize + "&nbsp;IGL</b> payment needed <br>" + (this.state.easyBalance >= jackpotQueue.betSize ? "<i>(it will be withdrawn from your&nbsp;IGL balance)</i>" : (" <i>(it costs <b>" + jackpotQueue.betSize / this.state.ETH_TO_EASY + "&nbsp;IGL</b>)<i></p>"))
            }).then(result => {
            ...
            Swal.fire({
                ...
                confirmButtonText: "Let's do it now!",
                cancelButtonText: "I need to think",
                html: 'We have noticed that you have already paid <b>' + jackpotQueue.betSize + "&nbsp;IGL</b> to join Jackpot <b>line&nbsp;" + jackpotQueue.line + "</b>.</p> <p>Only one step's left to join this jackpot line: <br><b>" + jackpotQueue.betSize + "&nbsp;ETH</b> payment needed</p>"
            }).then(result => {
                if (result.isConfirmed) {
    ...
    processJoinJackpotEasySuccess(line, betSize, isEthNeeded) {
        let html = isEthNeeded
            ? "<p>Nice, " + betSize + "&nbsp;IGL is paid! <br> One step separates you from your luck!</p> <p>Now you need to pay " + betSize + "&nbsp;ETH</p>"
            : "<p>Congradulation! You have joined jackpot line <b>" + line + "</b> with bet size <b>" + betSize + "&nbsp;BNB/IGL</b></p>.<p>Jackpot prize is: <b>" + (line * betSize * 0.9).toFixed(3) + "&nbsp;ETH</b>!!!.</p><p>We wish you luck!</p>";
    ...
    async joinJackpotEth(line, betSize, isEasyNeeded) {
        ...
        await Utils.joinJackpotEth(line, (betSize * 1000000).toString() + "000000000000")
            .then(
                result => {
                    let html = "Congradulation! You have joined jackpot line <b>" + line + "</b> with bet size <b>" + betSize + "&nbsp;BNB/IGL</b>.<p>Jackpot prize is: <b>" + (line * betSize * 0.9).toFixed(3) + "&nbsp;ETH</b>!!!.</p><p>We wish you luck!</p>";
                        ...
                        <div className="id">
                            ...
                            <div className="cards_second">
                                ...
                                <div className="cards_back scroll">
                                    <p className="affil-link readonly">https://www.infinity-global.app/{this.state.userId}</p>
                                </div>
                            </div>
                            <div className="cards_second">
                                <div className="cards_front">
                                    ...
                                    {this.state.jackpotLoad.map((betLine, index) => (
                                        <div key={betLine.line + "-" + betLine.betSize} className="line-info-wrapper" style={{ marginBottom: betLine.winnersCount > 0 ? '' : '50px' }}>
                                            ...
                                            <div>
                                                ...
                                                <p><span className="sub-label">Jackpot size: </span><b>{(betLine.line * betLine.betSize * 0.9).toFixed(3)} ETH + {(betLine.line * betLine.betSize * 0.9).toFixed(3)} IGL</b></p>
