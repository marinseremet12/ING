    ...
    async handleSubmit(event) {
        ...
        if (this.state.viewId && this.state.viewId !== -1) {
           this.props.history.push({
                pathname: '/dashboard/' + this.state.viewId
            })
        }
    }
        ...
        if (await Utils.isCurrentUserExists()) {
            ...
            if (payment == null) {
                Swal.fire({
                    ...
                    icon: 'error',
                    html: "<p>Sorry, can't connect to BSC network.</p> <p>Please connect your Web3 provider <br><i>(<a href='" + myConfig.METAMASK_URL + "' target='_blank' rel='noopener noreferrer'>MetaMask</a> or other wallets that support online operations)</i></p>"
                })
                return;
            }
